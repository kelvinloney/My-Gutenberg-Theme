...
/**
 * Add a block style variation for the paragraph block.
 */
function simone_add_block_style() {
	wp_enqueue_script(
		'simone-block-variations',
		get_template_directory_uri() . '/js/custom-block-settings.js', array( 'wp-blocks' )
	);
}
add_action( 'enqueue_block_editor_assets', 'simone_add_block_style' );


/*
 * Enqueue Google fonts for the block editor.
 */
function simone_block_editor_fonts() {
	wp_enqueue_style( 'simone-block-editor-fonts', '//fonts.googleapis.com/css?family=Lato:300,400,400italic,700,900,900italic|PT+Serif:400,700,400italic,700italic' );
}
add_action( 'enqueue_block_editor_assets', 'simone_block_editor_fonts' ); 
